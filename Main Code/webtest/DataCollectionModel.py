...
class DataCollection:
    ...
    def kospi_collection(self):
        ...
        self.model_kospi = self.model_kospi.drop(self.model_kospi[self.model_kospi['Labeling'] == 2].index)  # 전일 주가 = 당일 주가인 행 삭제
        ...
        self.scaling_col_2 = [column for column in
                         self.model_kospi.columns.difference(['Labeling', 'Open', 'High', 'Low', 'Close', 'Volume'])]

        self.model_kospi = self.model_kospi.dropna()
        self.model_kospi_2 = self.model_kospi.copy()
        self.model_kospi_2['Date'] = self.model_kospi_2.index
        self.model_kospi_2 = self.model_kospi_2[['Date'] + self.model_kospi_2.columns[:-1].tolist()]
        #self.model_kospi = DataCollection.feature_scaling(self.model_kospi, scaling_strategy="z-score", column=self.scaling_col)
        #self.model_kospi_2 = DataCollection.feature_scaling(self.model_kospi_2, scaling_strategy="z-score", column=self.scaling_col_2)
        #self.model_kospi.to_csv("model_kospi.csv", mode='w', index=False)
        # self.model_kospi_2.to_csv("model_kospi_2.csv", mode='w', index=False)
        ...
        self.ts_2 = self.model_kospi_2.loc[self.cd]
        self.ms_2 = self.model_kospi_2.append(self.ts_2, ignore_index=False)
        self.result_ = []
        # self.result_ : 분석이 아닌, 예측에 활용되는 데이터를 추가로 수집
        # self.result_[0] : 차트 데이터를 포함한 스케일링이 적용된 데이터
        # self.result_[1] : 차트 데이터를 포함하지 않은 스케일링이 적용된 데이터 (강화학습에 사용)
        # self.result_[2] : 예측 직전 데이터 (prophet, AutoML_REG에 사용)
        # self.result_[3] : 예측 직전 데이터 (강화학습에 사용)
        self.result_.append(self.model_kospi)
        self.result_.append(self.ms.loc[self.cd])
        self.result_.append(self.model_kospi_2)
        self.result_.append(self.ms_2.loc[self.cd])
    ...
    def yg_collection(self):
        ...
        self.model_yg = self.model_yg.drop(self.model_yg[self.model_yg['Labeling'] == 2].index)  # 전일 주가 = 당일 주가인 행 삭제
        ...
        self.scaling_col_2 = [column for column in
                         self.model_yg.columns.difference(['Labeling', 'Open', 'High', 'Low', 'Close', 'Volume'])]

        self.model_yg = self.model_yg.dropna()
        self.model_yg_2 = self.model_yg.copy()
        self.model_yg_2['Date'] = self.model_yg_2.index
        self.model_yg_2 = self.model_yg_2[['Date'] + self.model_yg_2.columns[:-1].tolist()]
        #self.model_yg = DataCollection.feature_scaling(self.model_yg, scaling_strategy="z-score", column=self.scaling_col)
        #self.model_yg_2 = DataCollection.feature_scaling(self.model_yg_2, scaling_strategy="z-score", column=self.scaling_col_2)
        #self.model_yg.to_csv("model_yg.csv", mode='w', index=False)
        # self.model_yg_2.to_csv("model_yg_2.csv", mode='w', index=False)

        self.cd = self.ed_date.strftime("%Y-%m-%d")
        self.ts = self.model_yg.loc[self.cd]
        self.ms = self.model_yg.append(self.ts, ignore_index=False)
        self.ts_2 = self.model_yg_2.loc[self.cd]
        self.ms_2 = self.model_yg_2.append(self.ts_2, ignore_index=False)
        self.result_ = []
        # self.result_ : 분석이 아닌, 예측에 활용되는 데이터를 추가로 수집
        # self.result_[0] : 차트 데이터를 포함한 스케일링이 적용된 데이터
        # self.result_[1] : 차트 데이터를 포함하지 않은 스케일링이 적용된 데이터 (강화학습에 사용)
        # self.result_[2] : 예측 직전 데이터 (prophet에 사용)
        # self.result_[3] : 예측 직전 데이터 (강화학습에 사용)
        self.result_.append(self.model_yg)
        self.result_.append(self.ms.loc[self.cd])
        self.result_.append(self.model_yg_2)
        self.result_.append(self.ms_2.loc[self.cd])
