...
class Agent:
    ...
    def validate_action(self, action):
        if action == Agent.ACTION_BUY:
            ...
            if self.balance < self.environment.get_price() * (
                ...
                #print("self.environment.get_price()", self.environment.get_price())
                #print("self.TRADING_CHARGE", self.TRADING_CHARGE)
                #print("self.min_trading_unit", self.min_trading_unit)
                #print("우변",self.environment.get_price() * (
                #1 + self.TRADING_CHARGE) * self.min_trading_unit)
                return False
        elif action == Agent.ACTION_SELL:
            # 주식 잔고가 있는지 확인 
            if self.num_stocks <= 0:
                #print("self.num_stocks", self.num_stocks)
                return False
        return True
    ...
    def act(self, action, confidence):
        ...
        self.create_action = 0  # 액션 (1=매수, 2=매도, 3=관망)
        ...
        # 매도
            ...
            if invest_amount > 0:
                ...
                self.create_action = 2

        # 홀딩
        elif action == Agent.ACTION_HOLD:
            self.num_hold += 1  # 홀딩 횟수 증가

            self.create_action = 3

        #print(self.environment.observation['Date'], 'action :', self.action)
        self.action_list.append([self.environment.observation['Date'], self.create_action])
